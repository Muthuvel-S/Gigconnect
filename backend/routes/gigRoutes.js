...
router.get('/client/stats', auth, async (req, res) => {
    try {
        ...
        const completed = await Gig.countDocuments({ postedBy: userId, status: { $in: ['completed', 'paid'] } });
        const inProgress = await Gig.countDocuments({ postedBy: userId, status: 'in progress' });
        const active = await Gig.countDocuments({ postedBy: userId, status: 'open' });
...
router.get('/freelancer/stats', auth, async (req, res) => {
    try {
        const userId = req.user.id;
        const completed = await Gig.countDocuments({ hiredFreelancer: userId, status: { $in: ['completed', 'paid'] } });
        const inProgress = await Gig.countDocuments({ hiredFreelancer: userId, status: 'in progress' });
        const earningsResult = await Gig.aggregate([
            { $match: { hiredFreelancer: new mongoose.Types.ObjectId(userId), status: 'paid' } },
...
// Client: My Gigs (UPDATED with review status)
...
router.get('/hired-freelancers', auth, async (req, res) => {
    try {
        ...
        const gigs = await Gig.find({ postedBy: req.user.id, status: 'in progress' })
            .populate('hiredFreelancer', 'username profilePicture');
        res.json(gigs);
    } catch (err) {
...
// ===========================
// PROPOSALS ROUTES (NEWLY ADDED)
// ===========================

// GET all proposals for a specific gig
...
// ===========================
// PROPOSAL ACTIONS (ACCEPT / REJECT)
// ===========================
...
router.put('/:gigId/proposals/:proposalId/accept', auth, async (req, res) => {
    ...
    try {
        ...
        if (!gig || gig.postedBy.toString() !== req.user.id)
            return res.status(403).json({ message: 'Not authorized to accept proposals for this gig.' });
...
router.put('/:gigId/proposals/:proposalId/reject', auth, async (req, res) => {
    ...
    try {
        const gig = await Gig.findById(gigId);
        if (!gig || gig.postedBy.toString() !== req.user.id)
            return res.status(403).json({ message: 'Not authorized to reject proposals for this gig.' });
        if (gig.status !== 'open') return res.status(400).json({ message: 'Gig is not open for proposals.' });
        const proposal = await Proposal.findById(proposalId);
        if (!proposal || proposal.gig.toString() !== gigId) return res.status(404).json({ message: 'Proposal not found.' });
...
router.post('/:id/review', auth, async (req, res) => {
    ...
    try {
        const gig = await Gig.findById(gigId);
        if (!gig) return res.status(404).json({ message: 'Gig not found.' });
        if (gig.status !== 'completed' && gig.status !== 'paid') {
            return res.status(400).json({ message: 'You can only review completed or paid gigs.' });
        }
        if (!gig.postedBy || !gig.hiredFreelancer) {
            return res.status(400).json({ message: 'Cannot review a gig without a client and a hired freelancer.' });
        }
        if (gig.postedBy.toString() !== reviewerId && gig.hiredFreelancer.toString() !== reviewerId) {
...
router.delete('/:id', auth, async (req, res) => {
    try {
        ...
        if (gig.status !== 'open') {
            return res.status(400).json({ message: 'Cannot delete a gig that is in progress or has been completed.' });
        }
